...
namespace PDollarGestureRecognizer
{
	...
	{
		public static void LoadPremadeGestureTemplates (string folderPath, List<Gesture> gestureList)
		{
			TextAsset[] gesturesXml = Resources.LoadAll<TextAsset> (folderPath);

			foreach (TextAsset gestureXml in gesturesXml)
				gestureList.Add (GestureIO.ReadGestureFromXML (gestureXml.text));
		}
		...
		public static void LoadCustomGestureTemplates (List<Gesture> gestureList)
		{
			...
			foreach (string filePath in filePaths)
				gestureList.Add (GestureIO.ReadGestureFromFile (filePath));
		}

		public static void SaveCustomGestureTemplate (string newGestureName, Gesture newGesture)
		{
			string fileName = String.Format ("{0}/{1}-{2}.xml", Application.persistentDataPath, newGestureName, DateTime.Now.ToFileTime ());

			#if !UNITY_WEBPLAYER
			GestureIO.WriteGesture (newGesture.Points, newGestureName, fileName);
			#endif
		}
